...
def find_centroid(cluster):
    ...
    return [mean(latitude(cluster)), mean(longitude(cluster))]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(r) for r in restaurant_list]
        n += 1
    return centroids
...
def rate_all(user, restaurants, feature_fns):
    ...
    """
    predictor = best_predictor(user, restaurants, feature_fns)
    restaurants_reviews = user_reviewed_restaurants(user, restaurants)
    user = {}

    for x in restaurants.keys():
        if x in restaurants_reviews.keys():
            new_dictionary={x: user_rating(user,x)}
            user.update(new)

        else:
            new_dictionary ={x:predictor(restaurants.get(x))}
            user.update(new)
